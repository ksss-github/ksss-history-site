...
export default function Timeline({ timelineData }) {
  ...
  // OBSERVER DETECTS WHEN A DECADE ELEMENT IS IN THE CENTER OF THE SLIDER AND SETS IT AS CURRENT DECADE ON SLIDER
  useEffect(() => {
    if (!refs1) return
    let timeoutId = null
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            timeoutId = setTimeout(() => {
              setCurrentDecadeOnSlider(entry.target.id)
            }, 500)
          }
        })
      },
    ...
    Object.values(refs1).forEach((ref) => {
      if (ref.current) {
  ...
  }, [refs1, setCurrentDecadeOnSlider])
    ...
    <div>
      ...
      <div className=" bg-[#fff] flex flex-col relative">
        ...
        <div className="relative bg-transparent h-[15rem] text-white flex flex-col justify-center">
          <RulerLines
            shortLinesCount={shortLinesCount}
            longLinesCount={longLinesCount}
          />
          <div
            style={{
              display: "grid",
            ...
            {decades
              ? decades.map((year) => (
                  <li
                    ...
                    id={year}
                    className="text-center list-none cursor-pointer text-white snap-center"
                    ref={refs1[year]}
          ...
          <div className="flex justify-center bg-[#162743] absolute top-0 left-0 bottom-0 right-0">
            <div className=" bg-red-600 w-[1px]"></div>
